"""
Builds Foursquare visitor crowd-density features by heat-distance bucket.
...
Outputs:
  data/foursquare_heat_bucket_estimates.csv -- per (date, market, brand,
    heat_distance_bucket), tagged certain vs split_estimate. Carries
    latitude/longitude: the POI's own for certain rows, the centroid of the
    candidate POIs sharing that bucket for split_estimate rows (not a real
    address).
  ...
  data/foursquare_heat_bucket_market_wide.csv -- Dallas + Houston pooled back
    ...
    (state_fk, market_fk, time_fk) so clean.py can merge
    it straight onto the market-day history it already builds. Each
    foursquare_crowd_<bucket> column has a matching foursquare_lat_<bucket> /
    foursquare_lon_<bucket>: the visitor-weighted centroid of that bucket's
    POIs that day (NaN, not 0, where the bucket had no visitors -- see
    weighted_bucket_centroids).
"""
...
def split_estimates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Centroid of the candidate POIs sharing this bucket -- there's no single
    # true location for a split estimate, so this is a representative point,
    # not a real address.
    bucket_group = ambiguous.groupby(GROUP_KEY + ["heat_distance_bucket"], observed=True)
    ambiguous["latitude"] = bucket_group["latitude"].transform("mean")
    ambiguous["longitude"] = bucket_group["longitude"].transform("mean")
...
def weighted_bucket_centroids(estimates: pd.DataFrame) -> pd.DataFrame:
    """Visitor-weighted centroid lat/lon per (date, heat_distance_bucket),
    pooled across brands/markets the same way build_market_wide pools crowd
    counts. Left as NaN (not 0) where a bucket has no visitors that day --
    unlike crowd count, 0.0/0.0 would misread as a real coordinate (Gulf of
    Guinea) instead of "no data"."""
    weighted = estimates.assign(
        lat_w=estimates["latitude"] * estimates["allocated_visitor_count"],
        lon_w=estimates["longitude"] * estimates["allocated_visitor_count"],
    )
    grouped = weighted.groupby(["date", "heat_distance_bucket"], observed=True).agg(
        lat_w=("lat_w", "sum"), lon_w=("lon_w", "sum"), weight=("allocated_visitor_count", "sum")
    )
    return pd.DataFrame(
        {"latitude": grouped["lat_w"] / grouped["weight"], "longitude": grouped["lon_w"] / grouped["weight"]}
    )
...
def build_market_wide(estimates: pd.DataFrame) -> pd.DataFrame:
    ...
    centroids = weighted_bucket_centroids(estimates)
    for coord, prefix in (("latitude", "foursquare_lat"), ("longitude", "foursquare_lon")):
        coord_wide = centroids[coord].unstack("heat_distance_bucket")
        coord_wide = coord_wide.rename(columns=lambda bucket, prefix=prefix: f"{prefix}_{HEAT_DISTANCE_COLUMN_SUFFIXES[bucket]}")
        wide = wide.join(coord_wide)
