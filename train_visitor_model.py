...
Compares regression models that predict a single store's visitor_count on
a single day, using data/visitor_daily_store_clean.csv

Notes:

- Target is log1p(visitor_count) -- the raw counts are heavily
  right-skewed (skew ~4.9, max 28,508) with 8.1% exact zeros, which log1p
  handles natively. Metrics are converted back via expm1 so they stay
  readable as visitors/day.

- store_id_fk, brand_name_fk, and category_fk are excluded from the
  feature set (see NON_FEATURE_COLUMNS in clean_visitor_daily_store.py);
  category_grouped / brand_name_grouped stand in for the latter two.

- The train/test split is chronological (train 2020-2023, test 2024), not
  random, so no model gets to peek at a store's neighboring days. As a
  side effect this also tests whether `year` helps or just risks
  extrapolation -- see run_year_ablation() below.

- The ~0.49% of rows missing lag/rolling history (each market's first few
  days) are dropped rather than imputed.
...
from clean_visitor_daily_store import NON_FEATURE_COLUMNS
...
assert not (set(FEATURE_COLUMNS) & set(NON_FEATURE_COLUMNS)), (
    "a column flagged NON_FEATURE_COLUMNS in clean_visitor_daily_store.py snuck into the feature set"
)
...
def encoded_pipeline(estimator) -> Pipeline:
    """ColumnTransformer + estimator for models with no native categorical
    support (Ridge, Lasso, RandomForest): one-hot for linear models needs
    scaling to regularize fairly across features, so this is shared
    plumbing parameterized by the final estimator."""
...
def ordinal_pipeline(estimator) -> Pipeline:
    ...
    search over ~440 sparse dummy columns for no accuracy benefit -- plain
    ordinal codes are the standard tree-model shortcut."""
    pre = ColumnTransformer(
        [
...
def bakeoff(train: pd.DataFrame, test: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for name, model in models.items():
        if name in ("HistGradientBoosting", "XGBoost", "LightGBM", "CatBoost"):
...
def run_year_ablation(train: pd.DataFrame, test: pd.DataFrame, winner_name: str) -> pd.DataFrame:
    ...
    clean_visitor_daily_store.py: refit the winning model type with and
...
def main() -> None:
    ...
    if winner_name in ("HistGradientBoosting", "XGBoost", "LightGBM", "CatBoost"):
